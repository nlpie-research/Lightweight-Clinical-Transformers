...
tokenizerPath = "bert-base-cased" #use other tokenisers e.g. "bert-base-uncased" based on your models
...
def load_datasets(info):
  ...
  if info.type == "classification":
    ...
    num_labels = len(set(list(np.concatenate([np.array(item).reshape(-1) for item in dataset["train"]["ner_tags"]], axis=0))))
...
def evaluate(model, info, valDataset, data_collator):
  model.eval()
    ...
    for batch_size in dsInfo.batch_size:
      for _ in range(dsInfo.runs):
        ...
        for name, dataset in zip(dsInfo.validationSubsets, valDatasets):
          ...
          result = evaluate(model, dsInfo, dataset, data_collator)
          with open(logsPath, mode="a+") as f:
            f.write(f"---HyperParams---\nBatchsize= {batch_size} Lr= {lr}\n---{name} results---\n{str(result)}\n\n")


datasets = [
    DatasetInfo("clinical-models/datasets/ClinicalDatasets/cnotes-multi-final", #The internal cancer identification (ICN) dataset used in our experiments.
                validationSubsets=["validation"],
                metric="f1-macro",
                load_from_disk=True,
                type="classification",
                isMultiSentence=False,
                ...
                batch_size=[16, 32],
                runs=1),
    DatasetInfo("clinical-models/datasets/ClinicalDatasets/i2b2-2010", #The Relation Extraction version of the i2b2-2010 dataset. Use the pre-processing code used in (https://github.com/EmilyAlsentzer/clinicalBERT)
                validationSubsets=["validation"],
                metric="f1-micro",
                load_from_disk=True,
                type="classification",
                ...
                batch_size=[16],
                runs=1),
    DatasetInfo("clinical-models/datasets/ClinicalDatasets/MedNLI",
